...
// Same as Test Case 2, but entities are set to silent immediately after creation.


void TestCase3::setup() {
   m_state = 0;

   m_entities.push_back(pEntity_t(new Entity(internString("BigShape0"), internString("Polygon"))));
   m_entities.back()->setSilent(true);
...
void TestCase3::onKeyDown(int key) {
   switch (key) {
