...
Renderer::mode_t Renderer::m_mode = Renderer::TEXTURED_ALPHA;
...
// Renderer::setMatrix
//===========================================
void Renderer::setMatrix(const matrixElement_t* mat) {
   GL_CHECK(glUniformMatrix4fv(m_locMVP, 1, GL_FALSE, mat));
}
...
void Renderer::render() {
   ...
   if (m_coloursSet && (m_colCount != m_vertCount))
...
// Renderer::clear
//===========================================
void Renderer::clear(const colourElement_t* colour) {
   if (colour) {
      GL_CHECK(glClearColor(colour[0], colour[1], colour[2], colour[3]));
   }
   else {
